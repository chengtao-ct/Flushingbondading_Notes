...
graph TD
    ...
    %% scheduler_comm 内部四大模块 (ID去掉了 .c 避免语法崩溃)
    subgraph CommCenter ["scheduler_comm.c (公共后勤中心)"]
        direction TB
    ...
    %% 底层硬件依赖
    subgraph BottomArch ["底层调度策略 (scheduler_up.c 或 _mp.c)"]
    ...
    startup -.->|通常会调用| insert
    update_prio -.->|重插队列| remove
    update_prio -.->|重插队列| insert
    yield -.->|插入队尾| insert

    %% ================= 样式美化 =================
    %% 给大外框上色 (直接使用 style 非常安全)
    style CommCenter fill:#e3f2fd,stroke:#1565c0,stroke-width:2px
    style BottomArch fill:#fff3e0,stroke:#e65100,stroke-width:2px,stroke-dasharray: 5 5
    
    %% 给外部触发节点上色
    style UserAPI fill:#e8f5e9,stroke:#2e7d32
    style SysTick fill:#fce4ec,stroke:#c2185b
    
    %% 给内部小分组上色区分
    style Init_Group fill:#ffffff,stroke:#90caf9
    style State_Group fill:#ffffff,stroke:#90caf9
    style Time_Group fill:#ffffff,stroke:#90caf9
    style Prio_Group fill:#ffffff,stroke:#90caf9
...
- 问题：其实本质上是对suspend,ready,running三个状态的存储模式是怎么样的
