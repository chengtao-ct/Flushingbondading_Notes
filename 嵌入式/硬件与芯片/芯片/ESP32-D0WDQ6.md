...
GPIO 12	可用	⚠️ HS2_DATA2（SD卡）
GPIO 13	可用	HS2_DATA3（SD卡）
GPIO 14	可用	HS2_CLK（SD卡）
GPIO 15	可用	HS2_CMD（SD卡）
...
│                                                      │
│   ┌─ 摄像头专用（16个GPIO）────────────────────┐    │
│   │  GPIO 5,18,19,21,22,23,25,26,27           │    │
│   │  GPIO 32,34,35,36,39                      │    │
│   └────────────────────────────────────────────┘    │
│                                                      │
│   ┌─ SD卡专用（4个GPIO）──────────────────────┐     │
...
│                                                      │
│   ┌─ 真正自由可用的 GPIO ─────────────────────┐     │
│   │  GPIO 0（Boot）, GPIO 2（LED/SD）         │     │
│   │  GPIO 4（Flash LED/SD）, GPIO 33（LED）   │     │
│   └────────────────────────────────────────────┘    │
│                                                      │
│   ⛔ GPIO 16 → PSRAM，永远不要碰                      │
│   ⛔ GPIO 1,3 → 串口，调试用                          │
...
│                    [Antenna]             │
│                                          │
│  [OV2640 Camera FPC Connector]           │
│                                          │
│  GPIO0 ─ [RST Button]                   │
│  GPIO0 ─ [BOOT Button] ← 按住 + RST    │
│                进入烧录模式               │
│                                          │
│  GPIO 2 ─ [LED]（蓝色，低电平亮）       │
│  GPIO 4 ─ [Flash LED]（白色，可做闪光灯）│
│                                          │
│  [MicroSD Slot] ← 背面，SPI模式         │
│                                          │
│  Programming Pins:                       │
│  ┌─────┬──────┬────┬────┬────┬────┐     │
│  │ 5V  │ GND  │ TX │ RX │ IO0│ RST│     │
│  └─────┴──────┴────┴────┴────┴────┘     │
│     ↑                  ↑    ↑            │
│   供电               GPIO1 GPIO3        │
│                     (TXD) (RXD)         │
