...
与本文紧密关联的是 [[内存空间分配]] 中"有MMU vs 无MMU"的对比，建议结合阅读。
...
### 2.6 MMU vs MPU：嵌入式开发者必知的关键区别

```mermaid
flowchart TB
    subgraph MMU系统["有 MMU 的系统 (Cortex-A)"]
        APP1["App A<br/>VA: 0x0000-0xFFFF"] -->|"MMU翻译"| PA1["物理页<br/>分散在各处"]
        APP2["App B<br/>VA: 0x0000-0xFFFF"] -->|"MMU翻译"| PA2["物理页<br/>分散在各处"]
        PA1 --- DRAM["物理内存"]
        PA2 --- DRAM
    end

    subgraph MPU系统["有 MPU 的系统 (Cortex-M)"]
        TASK1["Task A<br/>PA: 0x2000_0000"] --> SRAM["SRAM"]
        TASK2["Task B<br/>PA: 0x2000_8000"] --> SRAM
    end

    style MMU系统 fill:#e3f2fd
    style MPU系统 fill:#fff3e0
```

| 特性 | MMU | MPU (Memory Protection Unit) |
|------|-----|-----|
| **地址翻译** | 虚拟地址 → 物理地址 | 不翻译，直接物理地址 |
| **粒度** | 页（通常 4KB） | 区域（可自定义大小，最小 32B） |
| **区域数量** | 理论无限（由页表项决定） | 通常 8 或 16 个区域 |
| **进程隔离** | 支持（每进程独立页表） | 不支持（共享物理地址空间） |
| **缺页异常** | 支持（按需分配物理页） | 不支持 |
| **内存属性** | 页表项中设置 Cache/Shareable | 区域属性寄存器中设置 |
| **典型芯片** | Cortex-A、x86、RISC-V (Linux) | Cortex-M0+/M3/M4/M7 |
| **典型系统** | Linux、Android、Windows | FreeRTOS、裸机 |
| **使能方式** | SCTLR.M bit | MPU_CTRL.ENABLE bit |

#### MPU 能做什么？
- 设置某段内存为**只读**（防止意外写入关键数据）
- 设置某段内存为**不可执行**（XN bit，防止代码注入攻击）
- **特权级分离**（用户态不能访问内核数据区）
- 配合 RTOS 实现**任务栈保护**
...
> [!tip] 如何选择？
> 需要运行 Linux → 必须有 MMU (Cortex-A)
> 只跑 RTOS 或裸机 → MPU 足够 (Cortex-M)
> 两者兼需 → Cortex-R 或带 MMU 的 Cortex-A 在安全场景下运行 RTOS
...
#### 3.1.1 关键问题：MMU 开启那一刻发生了什么？

MMU 开启是"瞬间切换"——同一条指令前后，CPU 看到的地址空间完全不同：

```asm
/* U-Boot 中开启 MMU 的典型代码 */

    /* 此刻：VA = PA，物理地址直接可见 */

    ldr     r0, =mmu_table          /* 页表物理地址 */
    mcr     p15, 0, r0, c2, c0, 0   /* 写入 TTBR0 */
    mcr     p15, 0, r0, c2, c0, 1   /* 写入 TTBR1 */

    /* 关键：开启 MMU 之前，必须先建立恒等映射 (Identity Mapping) */
    /* 即 VA 0x80000000 → PA 0x80000000 */
    /* 否则下一条指令的地址被"翻译"后就找不到代码了 */

    mrc     p15, 0, r0, c1, c0, 0   /* 读 SCTLR */
    orr     r0, r0, #1               /* 设置 M bit (使能 MMU) */
    mcr     p15, 0, r0, c1, c0, 0   /* ← 这条指令之后 MMU 生效 */

    /* 此刻：所有地址都要经过页表翻译 */
    /* 如果没有恒等映射，这里直接跑飞 */
```

**这就是为什么 U-Boot / Linux 启动时总是先建"恒等映射"再开 MMU。**

> [!danger] 常见错误
> 如果在开启 MMU 前没有建立恒等映射，PC 所在的地址会被 MMU 翻译到一个不确定的位置，
> CPU 取到垃圾指令 → 立即崩溃，且没有任何调试信息。
...
### 4.3 ARM32 vs ARM64 MMU 关键差异

| 特性 | ARM32 (Cortex-A7/A9) | ARM64 (Cortex-A53/A72) |
|------|---------------------|----------------------|
| 虚拟地址宽度 | 32 位（4GB 空间） | 48 位（256TB 空间） |
| 页表级数 | 2 级（PGD + PTE） | 4 级（PGD→PUD→PMD→PTE） |
| 页大小 | 4KB / 64KB | 4KB / 16KB / 64KB |
| TTBR 寄存器 | TTBR0 + TTBR1 | TTBR0_EL1 + TTBR1_EL1 |
| 异常级别 | SVC / USR / IRQ / FIQ | EL0 / EL1 / EL2 / EL3 |
| ASID 宽度 | 8 位（256 个进程） | 16 位（65536 个进程） |
| 典型应用 | STM32MP1、i.MX6 | 树莓派 4/5、RK3588 |

> [!note] ASID (Address Space ID)
> ASID 是页表项中的进程标识符，用于区分不同进程的 TLB 条目。
> 切换进程时，如果新进程的 ASID 在 TLB 中仍有缓存，就不需要刷新整个 TLB，
> 大大减少了进程切换的开销。
...
- [[内存空间分配]] — 有MMU vs 无MMU系统的完整内存布局对比
- [[DMA 与 Cache 一致性]] — MMU 页表属性中 Cache 策略配置的实际影响
- [[存储器总体认知]] — Cache/TLB 在存储层次中的位置
- [[内存_概览]] — 内存知识体系总览
- [多核并发的处理](../../操作系统与内核/多核并发的处理.md) — 多核场景下 TLB 一致性扩展
